...
export function Header() {
  return <header className="sticky top-0 z-50 border-b border-white/10 bg-black/80 backdrop-blur">
    <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
      ...
      <Link href="/location/" className="rounded-full bg-orange-500 px-4 py-2 text-sm font-bold text-black">상담 예약</Link>
    </div>
  </header>;
}

export function Footer() {
  return <footer className="border-t border-white/10 bg-zinc-950 px-4 py-10 text-sm text-zinc-400"><div className="mx-auto max-w-6xl"><p className="font-bold text-white">테오짐 PT 청라점</p><p className="mt-2">인천 서구 청라 지역 예약제 PT샵 · 대표 직접 수업 · 운동기록 관리</p><p className="mt-6">© TEO GYM. All rights reserved.</p></div></footer>;
}

export function CTA() { return <section className="mx-auto max-w-6xl px-4 py-12"><div className="rounded-3xl border border-orange-500/30 bg-orange-500 p-7 text-black md:p-10"><p className="text-sm font-bold uppercase tracking-widest">Reservation</p><h2 className="mt-3 text-3xl font-black md:text-5xl">지금 몸 상태와 목표부터 상담해보세요.</h2><p className="mt-4 max-w-2xl font-medium">첫 상담에서 체형, 운동 경험, 체중 변화, 생활패턴을 확인한 뒤 필요한 관리 방향을 안내합니다.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Button href={site.links.reservation} dark>무료 체험 상담 예약</Button><Button href={site.links.place}>네이버 플레이스 보기</Button></div></div></section> }

export function Button({ href, children, dark=false }: { href: string; children: React.ReactNode; dark?: boolean }) { return <Link href={href} className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-bold transition ${dark ? 'bg-black text-white hover:bg-zinc-900' : 'border border-black/20 bg-white/80 text-black hover:bg-white'}`}>{children}</Link> }
